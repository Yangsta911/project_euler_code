// Program that prints the first ten digits of the
// sum of the following 100 50 digit numbers
// Yang Xin
...
int main(void) {
    ...
    while (col >= 0) {
    	column_total = 0;
    	column_total = column_total + carry;
    	row = 0;
    	while (row < 100) {
    		column_total = column_total + array[row][col];
    		row ++;
    	}
    	carry = column_total / 10;
    	answer[col] = column_total % 10;
    	col --;
    }
    while (count >= 2) {
